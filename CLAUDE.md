...
- `apps/desktop` — Tauri 2 shell (Windows-only target). Registers `.dbml` file association; `single-instance` plugin forwards subsequent opens via a `dbml-open` event; first-launch argv is held in `PendingOpen` and drained by the `take_pending_open` IPC command. The webview is kept hidden until the frontend has applied theme + initial content (no white flash).
